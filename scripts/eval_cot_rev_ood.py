...
def eval_file(model, encode, decode, path, device="mps", n_eval=500):
    ...
    for ln in lines:
        ...
        a_rev, b_rev, _, gold_rev = m.groups()
        ...
        max_new = min(100, model.config.block_size - x.size(1))
        y = generate(model, x, max_new_tokens=max_new)
        gen = decode(y[0].tolist())

        pred_rev = parse_cot_pred(gen)
...
def print_stats(title, stats, examples):
    ...
    for p, g, pr, gen in examples:
        print(f"{p} gold={g} pred={pr} gen={gen[:120]}")


if __name__ == "__main__":
    DEVICE = "mps"

    dataset_dir = Path("data/addition_rev/cot")
    out_dir = Path("out/addition_rev_cot_b128")

    encode, decode = load_char_meta(dataset_dir)
    model = load_model(out_dir, DEVICE)

    stats2, ex2 = eval_file(
        model, encode, decode,
        Path("data/addition_rev/test2_cot.txt"),
        device=DEVICE
    )

    stats3, ex3 = eval_file(
        model, encode, decode,
        Path("data/addition_rev/test3_cot.txt"),
        device=DEVICE
    )

    print_stats("REVERSED COT TEST 2-DIGIT", stats2, ex2)
    print_stats("REVERSED COT TEST 3-DIGIT", stats3, ex3)