...
def eval_file(model, encode, decode, path, device="mps", n_eval=500):
    ...
    for ln in lines:
        ...
        if not m:
            continue
        a, b, gold = int(m.group(1)), int(m.group(2)), m.group(3)
        prompt = f"{a}+{b}="
...
if __name__ == "__main__":
    DEVICE = "mps"
    dataset_dir = Path("data/addition_exp/direct")
    out_dir = Path("out/addition_exp_direct_b128")

    encode, decode = load_char_meta(dataset_dir)
    model = load_model(out_dir, DEVICE)

    stats2, ex2 = eval_file(model, encode, decode, Path("data/addition_exp/test2_direct.txt"), device=DEVICE)
    stats3, ex3 = eval_file(model, encode, decode, Path("data/addition_exp/test3_direct.txt"), device=DEVICE)

    print_stats("DIRECT TEST 2-DIGIT", stats2, ex2)
    print_stats("DIRECT TEST 3-DIGIT", stats3, ex3)
